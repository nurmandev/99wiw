...
export type LevelTableComponentPropsType = {
...
const LevelTableComponent: React.FC<LevelTableComponentPropsType> = ({
  columns,
  data,
  isLoading,
  selectRows,
  ...
  return (
    <div
      className={cn(
        'table-responsive relative sm:px-[20px] px-[5px] py-[5px] bg-transparent',
        containerClassName,
        styles.tableWrapper,
      )}
    >
      <table className={cn('table mb-0 min-h-[100px] w-full h-full', tableClassName, styles.table)}>
                  ...
                  return (
                    <th
                      ...
                      className={cn(
                        ...
                        'bg-color-card-bg-primary font-semibold whitespace-nowrap dark:text-white text-color-light-text-primary md:!text-[14px] sm:!text-[12px] !text-[10px] !border-0 text-left !px-[5px]',
                      )}
                      // {...{
                      //   style: {
        ...
        <tbody className={cn({ 'relative py-10': isLoading }, 'md:!text-[14px] sm:!text-[12px] !text-[10px]')}>
          {isLoading && (
            <tr>
          ...
          {!!data?.length &&
            getRowModel().rows.map((row) => (
              <tr
                ...
                className={cn(
                  renderRowClass?.(row),
                  'rounded-full bg-color-card-body-hover cursor-pointer hover:dark:bg-color-card-body-hover/80 hover:bg-color-light-hover-primary',
                )}
...
export default LevelTableComponent;
